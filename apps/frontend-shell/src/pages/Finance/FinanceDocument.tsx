...
export const FinanceDocument: React.FC = () => {
  const { t } = useTranslation();
  const traceId = generateTraceId();

  useEffect(() => {
    logger.info('FinanceDocument initialized', traceId);
  }, [traceId]);
  ...
  return (
    <DocumentLayout title={t('Journal Entry')}>
      <div id="header" style={sectionStyle}>
        <h3 style={sectionHeaderStyle}>{t('Header')}</h3>
        <div style={gridStyle}>
          <div style={fieldStyle}>
            <label style={labelStyle}>{t('Company Code')}</label>
            <input type="text" defaultValue="1000 - Global Corp" style={inputStyle} />
          </div>
          <div style={fieldStyle}>
          ...
          <div style={fieldStyle}>
            <label style={labelStyle}>{t('Document Date')}</label>
            <input type="date" defaultValue="2026-06-17" style={inputStyle} />
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>{t('Status')}</label>
            <input type="text" defaultValue={t('Unposted')} style={{...inputStyle, backgroundColor: '#f3f4f6', color: '#6b7280'}} readOnly />
          </div>
        </div>
      </div>
      ...
      <div id="accounting" style={sectionStyle}>
        <h3 style={sectionHeaderStyle}>{t('Accounting Information')}</h3>
        <div style={gridStyle}>
          <div style={fieldStyle}>
          ...
          <div style={fieldStyle}>
            <label style={labelStyle}>{t('Reference')}</label>
            <input type="text" defaultValue="Invoice 12345" style={inputStyle} />
          </div>
        </div>
      </div>
    </DocumentLayout>
